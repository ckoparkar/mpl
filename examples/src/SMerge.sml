  ...
  fun writeMergeSerial cmp (s1, s2) t =
    let
      ...
      fun loop i1 i2 j =
        if i1 = n1 then
          let
            val sl = (sliceIdxs s2 i2 n2)
          in Util.for (0, AS.length sl) (fn i => write (i+j) (AS.sub(sl, i)))
          end
        else if i2 = n2 then
          let
            val sl = (sliceIdxs s1 i1 n1)
          in Util.for (0, AS.length sl) (fn i => write (i+j) (AS.sub(sl, i)))
          end
        else
          let
...
(*
  fun writeMerge cmp (s1, s2) t =
    if AS.length t <= 4096 then
      ...
      Util.for (0, AS.length s2) (fn i => AS.update (t, i, (AS.sub(s2, i))))
    else
      let
        val n1 = AS.length s1
        val n2 = AS.length s2
